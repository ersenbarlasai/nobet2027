...
export const payrollEntrySchema=z.object({teacherSourceId:z.string().trim().min(1),dutyDate:isoDate,compensationTypeId:uuid,quantity:z.number().finite().refine(v=>v!==0),note:z.string().trim().max(500).nullable().optional()}).strict().refine(v=>v.quantity>0||Boolean(v.note?.trim()),{message:"Eksi düzeltme kaydı için açıklama zorunludur.",path:["note"]});
