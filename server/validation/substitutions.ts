...
export const createAbsenceSchema=z.object({teacherSourceId:z.string().trim().min(1),dateFrom:isoDate,dateTo:isoDate,absenceScope:z.enum(["all_day","selected_lessons"]),absenceTypeId:uuid,note:z.string().trim().max(500).nullable().optional(),lessons:z.array(z.object({assignmentDate:isoDate,timetableCardId:uuid}).strict()).min(1).max(500)}).strict().refine(v=>v.dateTo>=v.dateFrom,{message:"Bitiş tarihi başlangıçtan önce olamaz."});
export const setTaskSchema=z.object({teacherSourceId:z.string().trim().min(1).nullable(),unfilledNote:z.string().trim().max(500).nullable().optional(),expectedVersion:z.number().int().positive(),overrideAcknowledged:z.boolean().optional(),overrideNote:z.string().trim().max(500).nullable().optional(),mergeAcknowledged:z.boolean().optional(),mergeNote:z.string().trim().max(500).nullable().optional()}).strict();
