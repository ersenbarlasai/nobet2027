...
import { fetchAbsenceTypes } from "../lib/absenceTypes/api";
...
export default function DersYerineGorevlendirmePage() {
  ...
  const [form, setForm] = useState({ teacherSourceId: "", dateFrom: today(), dateTo: today(), absenceScope: "all_day" as Scope, absenceTypeId: "", note: "", selected: new Set<string>() });
  ...
  async function load() {
    setBusy(true);
    try {
      const [preparation, response, types] = await Promise.all([fetchSubPreparation(new URLSearchParams()), fetchSubLists(), fetchAbsenceTypes()]);
      setPrep(preparation); setLists(response.items);
      const activeTypes = types.items.filter((item) => item.isActive);
      setAbsenceTypes(activeTypes);
      setForm((value) => value.absenceTypeId || !activeTypes.length ? value : { ...value, absenceTypeId: activeTypes[0].id });
  ...
  async function saveAbsence() {
    ...
    try {
      await createAbsence({ teacherSourceId: form.teacherSourceId, dateFrom: form.dateFrom, dateTo: form.dateTo, absenceScope: form.absenceScope, absenceTypeId: form.absenceTypeId, note: form.note, lessons: selectedLessons.map((lesson) => ({ assignmentDate: lesson.assignmentDate, timetableCardId: lesson.timetableCardId })) });
  ...
  async function assign(candidate: SubCandidate, override = false, note: string | null = null, mergeAck = false, mergeNote: string | null = null) {
    if (!list || !task) return;
    try {
      await updateSubTask(task.id, { teacherSourceId: candidate.teacherSourceId, expectedVersion: list.version, overrideAcknowledged: override, overrideNote: note, mergeAcknowledged: mergeAck, mergeNote });
      await openList(list.id); showNotice(`${candidate.teacherName} görevlendirildi.${mergeAck ? " (Ders birleştirme — puantaja girmez.)" : ""}`, "success");
    } catch (error) {
      ...
      if (apiError.status === "daily_limit_acknowledgement_required") {
        ...
        if (reason) await assign(candidate, true, reason, mergeAck, mergeNote); return;
      }
      if (apiError.status === "lesson_merge_acknowledgement_required") {
        const reason = window.prompt(`${candidate.teacherName} öğretmenin kendi Kulüp dersiyle çakışıyor. Bu görev "ders birleştirme" olarak işaretlenecek ve puantaja (borç/eşik/ücret) girmeyecek. Onay gerekçesini yazın:`);
        if (reason) await assign(candidate, override, note, true, reason); return;
      }
  ...
  return <div className="sub-page">
    ...
    {view === "create" && <section className="sub-card sub-create-card"><div className="sub-section-heading"><span>1</span><div><h2>Yokluğu tanımlayın</h2><p>Öğretmeni ve yokluk aralığını seçip dersleri getirin.</p></div></div><div className="sub-form">
      ...
      <label>Yokluk türü<select value={form.absenceTypeId} onChange={(event) => setForm((value) => ({ ...value, absenceTypeId: event.target.value }))}><option value="">Seçin</option>{absenceTypes.map((item) => <option key={item.id} value={item.id}>{item.currentName}{item.currentCreatesDebt ? "" : " (borçtan muaf)"}</option>)}</select></label>
